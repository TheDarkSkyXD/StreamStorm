...
import type { TwitchGqlResponse } from "./twitch-types";
...
export async function fetchGamesForVideos(
  videoIds: string[]
  ...
  try {
    const response = await httpClient.fetch(GQL_ENDPOINT, {
      method: "POST",
      headers: {
    ...
    Object.values(json.data).forEach((videoData) => {
      ...
      const video = videoData as unknown as {
        id: string;
