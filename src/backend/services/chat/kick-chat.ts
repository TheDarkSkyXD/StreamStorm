...
export class KickChatService extends EventEmitter implements TypedEventEmitter {
    ...
    private setupConnectionHandlers(): void {
        ...
        this.pusher.connection.bind('error', (error: unknown) => {
            ...
            const errorObj = error as { type?: string; data?: { code?: number; message?: string }; error?: { type?: string; data?: { code?: number } } };
            ...
            if (errorObj?.type === 'PusherError' || errorObj?.error?.type === 'PusherError') {
                ...
                if (code && code >= 4000 && code < 4100) {
                    this.emit('error', new Error(`Pusher error ${code}: ${errorObj.data?.message || 'Unknown error'}`));
                }
    ...
    private setupChannelEventHandlers(
        ...
        pusherChannel.bind('App\\Events\\PinnedMessageDeletedEvent', (data: unknown) => {
