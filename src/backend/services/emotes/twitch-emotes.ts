...
class TwitchEmoteProvider implements EmoteProviderService {
    ...
    async fetchGlobalEmotes(): Promise<Emote[]> {
        ...
        try {
            const data = await api.get('https://api.twitch.tv/helix/chat/emotes/global', {
                headers: {
    ...
    async fetchEmoteSet(emoteSetId: string): Promise<Emote[]> {
        ...
        try {
            const data = await api.get(
                `https://api.twitch.tv/helix/chat/emotes/set?emote_set_id=${emoteSetId}`,
                {
                    headers: {
                        'Client-ID': this.clientId,
                        'Authorization': `Bearer ${this.accessToken}`,
    ...
    private transformEmote(
        ...
        return {
            ...
            owner: emote.owner_id
                ? { id: emote.owner_id, username: '', displayName: '' }
                : undefined,
        };
    }
}
