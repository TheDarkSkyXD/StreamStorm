...
class FFZEmoteProvider implements EmoteProviderService {
    ...
    async fetchGlobalEmotes(): Promise<Emote[]> {
        try {
            const data = await api.get(`${FFZEmoteProvider.BASE_URL}/set/global`).json<FFZGlobalResponse>();
    ...
    async fetchChannelEmotes(
        ...
        try {
            ...
            console.warn(`[FFZEmotes] Failed to fetch channel emotes for ${channelName || channelId}:`, error);
            return [];
        }
    }
    ...
    private transformEmote(emote: FFZEmote, isGlobal: boolean, channelId?: string): Emote {
        ...
        const urls = hasAnimated && emote.animated
            ? emote.animated
            : emote.urls;
        ...
        return {
            ...
            provider: 'ffz',
            isGlobal,
            isAnimated: hasAnimated ? true : false,
