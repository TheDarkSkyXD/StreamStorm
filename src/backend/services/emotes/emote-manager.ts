...
class EmoteManager extends EventEmitter {
    ...
    async loadGlobalEmotes(): Promise<void> {
        ...
        results.forEach((result, index) => {
            ...
            if (result.status === 'fulfilled') {
                console.log(`[EmoteManager] Loaded ${result.value.emotes.length} global emotes from ${providerName}`);
            } else {
                console.error(`[EmoteManager] Failed to load global emotes from ${providerName}:`, result.reason);
            }
        });
    }
    ...
    async loadChannelEmotes(
        ...
        const results = await Promise.allSettled(
            enabledProviders.map(async ([name, provider]) => {
                try {
                    ...
                    console.warn(`[EmoteManager] Failed to load channel emotes from ${name} for ${channelId}:`, error);
                    return { provider: name, emotes: [], fromCache: false };
                }
            })
        );
        ...
        results.forEach((result, index) => {
            ...
            if (result.status === 'fulfilled' && result.value.emotes.length > 0) {
                ...
                console.log(`[EmoteManager] Loaded ${result.value.emotes.length} channel emotes from ${providerName} for ${channelId} ${cacheStatus}`);
            }
        });
    }
    ...
    private enforceCacheLimits(): void {
        while (this.channelAccessOrder.length > MAX_CACHED_CHANNELS) {
            ...
            if (oldestChannelId) {
                console.debug(`[EmoteManager] LRU eviction: clearing emotes for channel ${oldestChannelId}`);
     ...
     */
    parseEmotes(text: string, channelId?: string): Array<{ type: 'text' | 'emote'; content: string | Emote }> {
    ...
    getStats(): { global: Record<string, number>; channels: Record<string, Record<string, number>> } {
        const stats: { global: Record<string, number>; channels: Record<string, Record<string, number>> } = {
            global: {},
            channels: {},
        };
