...
const getTabsForPlatform = (platform: 'twitch' | 'kick'): typeof TABS => {
    ...
    if (platform === 'twitch') {
        const twitchTabs = TABS.filter(t =>
            t.id === 'twitch' || t.id === 'bttv' || t.id === 'ffz' || t.id === '7tv'
        );
        return [...commonTabs, ...twitchTabs];
    } else {
        // Kick
        const kickTabs = TABS.filter(t =>
            t.id === 'kick' || t.id === '7tv'
        );
...
export const EmotePicker: React.FC<EmotePickerProps> = ({
    ...
    const emotesByProvider = useMemo(
        () => getEmotesByProvider(),
        [getEmotesByProvider, globalEmotesLoaded, loadedChannels, activeChannelId]
    );
                ...
                <div className="flex overflow-x-auto border-b border-[var(--color-border)] scrollbar-hide">
                    {availableTabs.map((tab) => (
                        <button
                            ...
                            className={`flex-shrink-0 px-3 py-2 text-xs font-medium transition-colors ${activeTab === tab.id
        ...
        >
            ...
            {showFavButton && (
                <button
                    ...
                    className={`absolute -top-1 -right-1 w-4 h-4 rounded-full flex items-center justify-center text-xs ${isFavorite
