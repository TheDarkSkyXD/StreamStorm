    ...
    const suggestions = useMemo(() => {
        ...
        const filtered = recentChatters
            ...
            .sort((a, b) => {
                ...
                const aStartsWithQuery = a.username.toLowerCase().startsWith(query) ||
                    a.displayName.toLowerCase().startsWith(query);
                const bStartsWithQuery = b.username.toLowerCase().startsWith(query) ||
...
const MentionAutocompleteItem: React.FC<MentionAutocompleteItemProps> = ({
    ...
    return (
        <div
            ...
            className={`flex items-center gap-2 px-3 py-1.5 cursor-pointer transition-colors ${isSelected ? 'bg-white/10' : 'hover:bg-white/5'
                }`}
            onClick={onClick}
