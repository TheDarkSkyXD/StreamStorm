...
import { ChatMessage as ChatMessageType, ContentFragment } from '../../shared/chat-types';
...
export const ChatMessage: React.FC<ChatMessageProps> = memo(({ message, style }) => {
    ...
    if (isDeleted) {
        return (
            <div
                className="px-4 py-1 text-sm text-gray-500 italic opacity-50"
                style={style}
            >
                Message deleted
            </div>
    ...
    return (
        ...
        >
            <div className="break-words">
                ...
                {/* Content */}
                <span className={`align-middle ${message.isAction ? 'italic' : ''}`} style={message.isAction ? { color: message.color } : undefined}>
...
const MessageFragment: React.FC<{ fragment: ContentFragment; platform: 'twitch' | 'kick' }> = memo(({ fragment, platform }) => {
